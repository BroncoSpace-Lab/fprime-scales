...
port, fed directly from `imx_hubComStub.comStatusOut` (a standard
`Svc::ComStub`/`Fw.SuccessCondition` port: `SUCCESS` on a genuine reconnect,
`FAILURE` the first time a send actually fails) -- this port was previously
wired to `imx_hubFramer.comStatusIn`, which only re-emitted it on its own
`comStatusOut`, a port nothing consumed; redirecting it to `JetsonManager`
costs nothing. `isJetsonHubLinkTrusted()` now additionally requires
`m_hubLinkConnected`, closing the race regardless of how or when the
